...
function Modifier({ label, name, item }: { label: string; name: SrcriptItemModifiers; item: SrcriptItemKey; }) {
    ...
    return (
        <div className={classNames("max-w-[10rem] flex items-center space-x-2")}>
            <div className="min-w-[8ch] text-xs">
                {label}
            </div>
...
export function Frame({ title, children }: HTMLAttributes<HTMLDivElement> & { title: string; children: ReactNode; }) {
    return (
        <div className="relative p-2 border-primary-400 dark:border-primary-700 border rounded flex flex-col space-y-1">
